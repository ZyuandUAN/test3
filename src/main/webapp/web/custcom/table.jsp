...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'//绑定容器
    ,url:con.app+'/custcom/list2'//数据接口//默认会自动传递两个参数：？page=1&limit=30
    ,height:'full-230'//容器高度
    ,request:{
    	pageName:'pageIndex'//页码的参数名称（从第几页开始）
    	,limitName:'pageLimit'//每页的数据量的参数名（一页放几个）
    }
    ,where:{userCode:$("input[name='userCode']").val(),custCode:$("input[name='custCode']").val()}
   ,page:true//分页开启
    ,cols: [[//表头
      ...
      ,{field:'type', width:100, title: '购买方式'}
      ,{field:'content', width:120, title: '评价'}
	...
	<script>
		function add(){
			layui.use('layer', function(){
				...
				layer.open({
					type:2,
					content:"${pageContext.request.contextPath}/web/custcom/add.jsp",
					area: ['700px', '600px'],
		...
		function text(idd){//修改的弹出框	
			layui.use('layer', function(){
				...
				layer.open({
					type:2,
					content:"${pageContext.request.contextPath}/web/custcom/upd.jsp?userCode="+idd,//userServlet?usercode="+idd+"&action=upd",
					area: ['700px', '600px'],
...
refresh();
		...
		function exportExcel(){
			window.location.href=con.app+'/custcom/exportExcel'
			}
