...
<body>
        ...
        <div class="layui-input-inline">
        <select name="custCode" ></select>
        </div>
        </div>
		<div class="layui-form-item">
        <label class="layui-form-label">商品名称</label>
        <div class="layui-input-inline">
        <select name="proName" ></select>
        </div>
        </div>
		      <div class="layui-form-item">
		     <label class="layui-form-label">销量</label>
		   <div class="layui-input-inline">
		       <input type="text" name="count" id="username" value="" required  lay-verify="required" placeholder="必填" autocomplete="off" class="layui-input">
		     </div>
		   </div>
		         <div class="layui-form-item">
 ...
 $.ajax({
						...
						success:function(data){
							...
							var ht="";
						 	$.each(data,function(i,dom){
							 	//console.log(dom)
							 	ht+='<option value="'+dom.userCode+'">'+dom.userCode+'</option>'
									$('select[name="userCode"]').html(ht);				
							});//each 
						 	form.render();
						}//ajax.success
					});//ajax
					 $.ajax({
							...
							success:function(data){
								...
								var st="";
							 	$.each(data,function(ii,d){
								 	console.log(d)
								 	st+='<option value="'+d.custCode+'">'+d.custCode+'</option>'
										$('select[name="custCode"]').html(st);				
								});//each 
							 	form.render();
							}//ajax.success
						});//ajax
						 $.ajax({
								type:'post',
								url:contextPath+'/proinfo/list',
								dataType:'json',
								data:{},
								success:function(data){
									//console.log(data)
									var st="";
								 	$.each(data,function(iii,d){
									 //	console.log(d)
									 	st+='<option value="'+d.proName+'">'+d.proName+'</option>'
											$('select[name="proName"]').html(st);				
									});//each 
								 	form.render();
								}//ajax.success
							});//ajax
