...
<label class="layui-form-label">商品编号</label>
 ...
 <input type="text" name="proCode" placeholder="请输入账号"autocomplete="off" class="layui-input"> 
</div>
<label class="layui-form-label">商品名称</label>
<div class="layui-input-inline">
 <input type="text" name="proName" placeholder="请输入职位"autocomplete="off" class="layui-input"> 
</div>
...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'//绑定容器
    ,url:con.app+'/proinfo/list2'//数据接口//默认会自动传递两个参数：？page=1&limit=30
    ,height:'full-230'//容器高度
    ,request:{
    	pageName:'pageIndex'//页码的参数名称（从第几页开始）
    	,limitName:'pageLimit'//每页的数据量的参数名（一页放几个）
    }
    ,where:{proCode:$("input[name='proCode']").val(),proName:$("input[name='proName']").val()}
   ,page:true//分页开启
    ,cols: [[//表头
       {title:'全选',type:'checkbox',fixed:'left'}//LAY_CHECKED:true
       ,{title:'序号',type:'numbers',fixed:'left'}
      ,{field:'id', title:'主键ID',width:80,sort: true,fixed:'left',align:'right'}
      ,{field:'proCode', width:120, title: '商品编号'}
      ,{field:'proName', title: '商品名称', Width: 120,sort:true}
      ,{field:'sum', width:100, title: '数量'}
      ,{field:'cost', width:100, title: '单价(万)'}
	...
	<script>
		function add(){
			layui.use('layer', function(){
				...
				layer.open({
					type:2,
					content:"${pageContext.request.contextPath}/web/proinfo/add.jsp",
					area: ['700px', '600px'],
		...
		function text(idd){//修改的弹出框	
			layui.use('layer', function(){
				...
				layer.open({
					type:2,
					content:"${pageContext.request.contextPath}/proinfo/toupd?proCode="+idd,//userServlet?usercode="+idd+"&action=upd",
					area: ['800px', '700px'],
		...
		function aa2(proCode,act){
			layui.use('layer', function(){
				  var layer = layui.layer;
				  if(confirm("确认真的要删除么？")){
						 $.ajax({
							type:'POST',
							url:'/test3/proinfo/del',
							dataType:"text",
...
refresh();
		...
		function exportExcel(){
			window.location.href=con.app+'/proinfo/exportExcel'
			}
...
<script type="text/html" id="barDemo">
			<button onclick="text('{{d.proCode}}')" class="layui-btn layui-btn-primary">
			<i class="layui-icon layui-icon-edit"></i>修改</button>
			<button onclick="aa2('{{d.proCode}}','del')" class="layui-btn layui-btn-danger">
